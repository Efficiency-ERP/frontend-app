...
export default function CreateIssuePage() {
  ...
  const { addIssue } = useInvoicesStore()

  const customers = useMemo(() => contacts.filter((c) => c.partyType !== "supplier"), [contacts])

  const [number, setNumber] = useState<string>(() => `BS-${Math.floor(Math.random() * 10000)}`)
  ...
  const addFreeformLine = () => {
    setLines((prev) => [...prev, { id: `il-${Date.now()}`, code: "", designation: "", unit: null, quantity: 1 }])
  }
  const addLineFromArticle = (articleId: string) => {
    const a = articles.find((x) => x.id === articleId)
    if (!a) return
    setLines((prev) => [...prev, { id: `il-${Date.now()}`, code: a.code, designation: a.designation, unit: a.unit, quantity: 1 }])
  }
  ...
  const save = () => {
    ...
    addIssue(i)
    navigate(`/dashboard/issues/${i.id}`)
  }
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Créer Bon de Sortie {number ? `— ${number}` : ""}</CardTitle>
          <CardDescription>Sortie de stock pour livraison</CardDescription>
        </CardHeader>
